...
std::vector<cell<T1,T2>> createCells(size_t cells_size, bool value_same=false){
...
template <typename T1, typename T2>
void writeFile(char * file_name, size_t key_num, bool value_same){
    ...
    keyList = createCells<T1,T2>(key_num,value_same);
...
int main(int argc, char** argv){
    size_t key_num;
    char * file_name;
    int int_value_same, int_read_write;
    bool value_same, read_write;
    ...
    if (read_write == true){
        writeFile<uint32_t,uint32_t>(file_name, key_num, value_same);
    }else{
