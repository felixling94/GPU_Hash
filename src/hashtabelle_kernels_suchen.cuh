#ifndef HASHTABELLE_KERNELS_SUCHEN_CUH
#define HASHTABELLE_KERNELS_SUCHEN_CUH
...
GLOBALQUALIFIER void kernel_hashsuchen_Linear(Knoten<uint32_t,uint32_t>* pKnoten, 
Knoten<uint32_t,uint32_t>* pHashtabelle,uint32_t pGroesseHashtabelle, 
Kollision<uint32_t,uint32_t,uint32_t> * pKollision){
   suchen_Linearhash<uint32_t,uint32_t,uint32_t>(pKnoten,pHashtabelle,pGroesseHashtabelle, pKollision);
}

GLOBALQUALIFIER void kernel_hashsuchen_Quadratisch(Knoten<uint32_t,uint32_t>* pKnoten, 
Knoten<uint32_t,uint32_t>* pHashtabelle,uint32_t pGroesseHashtabelle, 
Kollision<uint32_t,uint32_t,uint32_t> * pKollision){
   suchen_Quadratischhash<uint32_t,uint32_t,uint32_t>(pKnoten,pHashtabelle,pGroesseHashtabelle, pKollision);
}

GLOBALQUALIFIER void kernel_hashsuchen_Cuckoo(Knoten<uint32_t,uint32_t>* pKnoten, 
...
Kollision<uint32_t,uint32_t,uint32_t> * pKollision){
   suchen_Cuckoohash<uint32_t,uint32_t,uint32_t>(pKnoten,pHashtabelle1,pGroesseHashtabelle1,pHashtabelle2,pGroesseHashtabelle2, pKollision);
}

//Suche nach einem Array von Schlüssel in der Hashtabelle durch CUDA.
template <typename T0, typename T1, typename T2>
void Hashtabelle<T0,T1,T2>::suchen_CUDA(Knoten<T1,T2> * pKnoten, size_t pKnotenGroesse, Kollision<T0,T1,T2> * pKollision){
   if (hashtyp_kode == keine_aufloesung){
      //TODO
   }else if (hashtyp_kode == linear_aufloesung){
      //Reserviere und kopiere Daten aus der Hashtabelle, eingegebenen Knoten und Kollisionstabelle auf GPU
      ...
      cudaMalloc(&KollisionGPU,sizeof(Kollision<T0,T1,T2>)*groesseHashtabelle);
      ...
      cudaMemcpy(KollisionGPU,pKollision,sizeof(Kollision<T0,T1,T2>)*groesseHashtabelle,cudaMemcpyHostToDevice);
      ...
      kernel_hashsuchen_Linear<<<1,threads>>>(knotenGPU, hashtabelleGPU,groesseHashtabelle, KollisionGPU);
      ...
      cudaMemcpy(pKollision, KollisionGPU, sizeof(Kollision<T0,T1,T2>)*(size_t)groesseHashtabelle, cudaMemcpyDeviceToHost);
      ...
      std::cout << "GPU sucht nach " << pKnotenGroesse << " Datenelementen in der Hashtabelle in ";
      std::cout << Millisekunden << " Millisekunden (" << (pKnotenGroesse / (double) Sekunden / 1000000.0f);
      std::cout << " Millionen Schlüssel pro Sekunde) bei linearen Hashverfahren.";
      std::cout << "" << std::endl;
      ...
      kernel_hashsuchen_Quadratisch<<<1,threads>>>(knotenGPU, hashtabelleGPU,groesseHashtabelle, KollisionGPU);
      ...
      std::cout << "GPU sucht nach " << pKnotenGroesse << " Datenelementen in der Hashtabelle in ";
      std::cout << Millisekunden << " Millisekunden (" << (pKnotenGroesse / (double) Sekunden / 1000000.0f);
      std::cout << " Millionen Schlüssel pro Sekunde) bei quadratischen Hashverfahren.";
      std::cout << "" << std::endl;
...
//Suche nach einem Array von Schlüsseln durch CUDA und Cuckoo-Hashverfahren.
template <typename T0, typename T1, typename T2>
void suchen_Cuckoo_CUDA(Knoten<T1,T2> * pKnoten, size_t pKnotenGroesse,
...
Kollision<T0,T1,T2> * pKollision){
   ...
   kernel_hashsuchen_Cuckoo<<<1,threads>>>(knotenGPU,hashtabelleGPU1,usrGroesseHashtabelle1,hashtabelleGPU2,usrGroesseHashtabelle2,KollisionGPU);
   ...
   std::cout << "GPU sucht nach " << pKnotenGroesse << " Datenelementen in der Hashtabelle in ";
   std::cout << Millisekunden << " Millisekunden (" << (pKnotenGroesse / (double) Sekunden / 1000000.0f);
   std::cout << " Millionen Schlüssel pro Sekunde) bei Cuckoo Hashverfahren.";
   std::cout << "" << std::endl;
