...
int main(int argc, char** argv){
    ...
    std::vector<uint32_t> schluesselListe = erzeuge_schluessel_zelle<uint32_t>(maxSchluessel, usrGroesseSchluessel);
    std::vector<uint32_t> werteListe = erzeuge_werte_zelle<uint32_t>(maxSchluessel, usrGroesseSchluessel);
    
    Startuhr1 = beginn_ende();

    //Beginn des Tests
    std::cout << "Tests für Einfügung von " << usrGroesseSchluessel << " Datenelementen in die Hashtabelle" << std::endl;
