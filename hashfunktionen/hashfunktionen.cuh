...
namespace Hashfunktionen_Device{
    //Berechne den Hashwert eines Schlüssels durch Modulo-Funktion
    template <typename T>
    DEVICEQUALIFIER INLINEQUALIFIER size_t modulo_hash(T pSchluessel, size_t pGroesseHashtabelle){
    ...
    template <typename T>
    DEVICEQUALIFIER INLINEQUALIFIER size_t multiplikativ_hash(T pSchluessel, size_t pGroesseHashtabelle){
    ...
    DEVICEQUALIFIER INLINEQUALIFIER size_t perfekt_hash(T pSchluessel){
        size_t schluessel_hash = (size_t) pSchluessel;
    ...
    template <typename T>
    DEVICEQUALIFIER INLINEQUALIFIER size_t murmer_hash(T pSchluessel, size_t pGroesseHashtabelle){
        size_t schluessel_hash = (size_t) pSchluessel;
