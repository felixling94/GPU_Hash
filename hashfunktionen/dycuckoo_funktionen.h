#ifndef DYCUCKOO_FUNKTIONEN_H
#define DYCUCKOO_FUNKTIONEN_H
...
//Berechne den Hashwert eines Schlüssels durch verschiedene Hashfunktionen aus DyCuckoo
namespace DyCuckoo_Funktionen{
    template <typename T>
    size_t hash1(T pSchluessel){
    ...
    template <typename T>
    size_t hash3(T pSchluessel){
        size_t schluessel_hash = (size_t) pSchluessel;
        return ((schluessel_hash ^ 59064253) + 72355969) % PRIME_uint;
    };

    template <typename T>
    size_t hash4(T pSchluessel){
        size_t schluessel_hash = (size_t) pSchluessel;
